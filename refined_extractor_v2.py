"""
Advanced PDF Extraction Module for Scottish National 5 Exam Papers - Version 2.0
...
class AdvancedQuestionExtractor:
    ...
    def _post_process_questions(self, questions):
        ...
        for question in questions:
            ...
            # Ensure question number is a string and fix formatting
    ...
    def _remove_other_question_references(self, text):
        """
        Remove references to other questions from the text.
        ...
        Returns:
            str: Cleaned text
        """
        # Remove references to other questions (e.g., "2. A company...")
        text = re.sub(r'\d+\.\s+[A-Z]', lambda m: m.group(0)[-1], text)
        
        # Remove trailing question numbers
        text = re.sub(r'\s+\d+\.\s*$', '', text)
    ...
    def _clean_headers_footers(self, text):
        """
        Remove headers, footers, and irrelevant sections from text.
        ...
        """
        # Remove headers
        text = re.sub(r'FORMULAE LIST.*?(?=\d+\.)', '', text, flags=re.DOTALL)
        
        # Remove footers
        text = re.sub(r'ADDITIONAL SPACE FOR ANSWERS.*', '', text, flags=re.DOTALL)
        text = re.sub(r'\[BLANK PAGE\].*', '', text, flags=re.DOTALL)
        text = re.sub(r'DO NOT WRITE ON THIS PAGE.*', '', text, flags=re.DOTALL)
        
        # Remove page markers and other irrelevant text
        text = re.sub(r'MARKS\s+DO NOT\s+WRITE IN\s+THIS\s+MARGIN', '', text)
        text = re.sub(r'\[Turn over', '', text)
        text = re.sub(r'\[END OF QUESTION PAPER\]', '', text)
        
        return text
    
    def _clean_question_text(self, text):
        """
        Clean up question text by removing page markers, headers, etc.
