...
class AdvancedPDFExtractor:
    ...
    def extract_from_pdf(self, pdf_path, subject):
        ...
        questions = self._extract_questions_from_pages(pdf_reader, start_page, subject, diagrams)
    ...
    def _extract_questions_from_pages(self, pdf_reader, start_page, subject, diagrams):
        """
        Extract questions by directly parsing each page of the PDF.
        
        Args:
            pdf_reader: PyPDF2 PdfReader object
            start_page: First page to start extraction (after cover and formula)
            subject: Subject of the exam
            diagrams: Dictionary of diagram information
        ...
        for page_num in range(start_page, len(pdf_reader.pages)):
            ...
            # Look for question numbers at the beginning of lines
            # Pattern for main questions like "1.", "2.", etc.
            main_question_matches = re.finditer(r'(?:^|\n)\s*(\d+)\.\s+', text)
            
            for match in main_question_matches:
                question_number = match.group(1)
                start_pos = match.end()
                
                # Find the end of this question (next question or end of text)
                next_match = re.search(r'(?:^|\n)\s*\d+\.\s+', text[start_pos:])
                if next_match:
                    end_pos = start_pos + next_match.start()
                    question_text = text[start_pos:end_pos].strip()
                else:
                    question_text = text[start_pos:].strip()
                ...
                if sub_parts:
                    ...
                    for part_letter, part_text in sub_parts:
    ...
    def _extract_sub_parts(self, text):
        ...
        Returns:
            list: List of tuples (part_letter, part_text)
        """
        sub_parts = []
        
        # Pattern for sub-parts like "(a)", "(b)", etc.
        sub_part_matches = re.finditer(r'(?:^|\n)\s*\(([a-z])\)\s+', text, re.MULTILINE)
        
        for match in sub_part_matches:
            part_letter = match.group(1)
            start_pos = match.end()
            
            # Find the end of this part (next part or end of text)
            next_match = re.search(r'(?:^|\n)\s*\([a-z]\)\s+', text[start_pos:])
            if next_match:
                end_pos = start_pos + next_match.start()
