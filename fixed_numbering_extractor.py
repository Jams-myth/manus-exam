...
class AdvancedPDFExtractor:
    ...
    def extract_from_directory(self, input_dir, output_dir):
        ...
        for subject, questions in self.questions.items():
            if questions:
                # Fix question numbering for Applications of Mathematics
                if subject == "Applications_of_Mathematics":
                    questions = self._fix_applications_numbering(questions)
    ...
    def _fix_applications_numbering(self, questions):
        """
        Fix question numbering for Applications of Mathematics.
        ...
        for question in questions:
            ...
            parts = re.match(r'(\d+)\.(?:\(([a-z])\))?', question["question_number"])
            if parts:
    ...
    def _extract_questions_from_cleaned_text(self, cleaned_text, subject, diagrams):
        ...
        question_matches = re.finditer(question_pattern, cleaned_text)
        ...
        for i, (question_number, start_pos) in enumerate(question_positions):
            ...
            if i < len(question_positions) - 1:
                end_pos = question_positions[i + 1][1] - len(question_positions[i + 1][0]) - 1
            else:
            ...
            if sub_parts:
                ...
                for part_letter, part_start, part_end in sub_parts:
                    # Remove sub-part text from main text
                    sub_text = question_text[part_start:part_end]
                    main_text = main_text.replace(sub_text, '').strip()
                
                # Process each sub-part
                for part_letter, part_start, part_end in sub_parts:
                    part_text = question_text[part_start:part_end].strip()
    ...
    def _extract_sub_parts(self, text):
        ...
        sub_part_matches = re.finditer(sub_part_pattern, text)
        ...
        for match in sub_part_matches:
            ...
            start_pos = match.end()
            sub_part_positions.append((part_letter, start_pos))
        
        # Process each sub-part
        for i, (part_letter, start_pos) in enumerate(sub_part_positions):
            # Determine end position (start of next sub-part or end of text)
            if i < len(sub_part_positions) - 1:
                end_pos = sub_part_positions[i + 1][1] - len(sub_part_positions[i + 1][0]) - 3  # Adjust for "(x)" pattern
            else:
                end_pos = len(text)
            
            sub_parts.append((part_letter, start_pos, end_pos))
